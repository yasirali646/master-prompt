...
import { createImportStream } from "@/lib/generator";
...
export async function POST(request: Request) {
  ...
  const existingPrompt = (body.prompt as string)?.trim();
  if (!existingPrompt || existingPrompt.length < 20) {
    return Response.json({ detail: "Prompt must be at least 20 characters" }, { status: 400 });
  }
  ...
  try {
    ...
    const result = createImportStream(existingPrompt, config, {
      provider,
      model,
