...
import { createRefineStream } from "@/lib/generator";
...
export async function POST(request: Request) {
  ...
  const { idea, currentOutput, instruction } = body;
  if (!idea || !currentOutput || !instruction) {
    return Response.json({ detail: "idea, currentOutput, and instruction required" }, { status: 400 });
  }
  ...
  try {
    ...
    const result = createRefineStream(idea, currentOutput, instruction, config, {
      provider,
      model,
    ...
    return Response.json(
      { detail: err instanceof Error ? err.message : "Refine failed" },
      { status: 400 }
    );
  }
}
